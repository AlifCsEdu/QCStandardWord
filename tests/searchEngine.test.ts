...
import { BASE_ITEMS, CODE_SUBS } from '../src/data/qcData.ts';
import { isApprox, lev, searchQCItems, subseq } from '../src/utils/searchEngine.ts';

describe('Milestone 2 Search Engine Unit Tests (tests/searchEngine.test.ts)', () => {
  describe('Algorithm Primitives', () => {
    it('bounded Levenshtein distance lev(a, b, cap)', () => {
      ...
      assert.equal(lev('screen', 'scr', 2), 3);
    });
